...
class LogBleedRegularizer:
    ...
    def track(self, toks):
        """
        Store 3-token patterns in a dict.
        Works for:
            • 1-D tensor, e.g.  (seq_len,)
            • 2-D tensor, e.g.  (batch, seq_len)
            • list/tuple with any nesting depth 1
        """
        # ---- 1. flatten to a plain Python list[int]  --------------------------
        if isinstance(toks, torch.Tensor):
            toks_cpu = toks.view(-1).cpu().tolist()          # tensor → 1-D list
        elif isinstance(toks[0], (list, tuple)):
            toks_cpu = [t for sub in toks for t in sub]      # list-of-lists
        else:
            toks_cpu = list(toks)                            # already flat
        # ----------------------------------------------------------------------

        # Need at least 3 tokens to form a 3-gram
        if len(toks_cpu) < 3:
            debug_print(f"LogBleed: Not enough tokens to track ({len(toks_cpu)} < 3)")
            return

        # ---- 2. add 3-grams to the histogram ---------------------------------
        for i in range(len(toks_cpu) - 2):
            p = tuple(toks_cpu[i : i + 3])                   # tuple of ints
            self.hist[p] = self.hist.get(p, 0) + 1
        # ----------------------------------------------------------------------

        # Keep the dict from growing without bound
        if len(self.hist) > self.max_history_size:
            top = sorted(self.hist.items(), key=lambda x: x[1], reverse=True)
            self.hist = dict(top[: self.max_history_size])
...
# ––– NEW: ENTROPY WINDOW –––
class EntropyWindow:
    def __init__(self, cap=8, thresh=0.18):
        self.cap, self.thresh = cap, thresh
        self.buf: List[float] = []

    def update(self, logits: torch.Tensor) -> bool:
        # unbiased variance of token-level distribution
        var = logits.var(dim=-1).mean().item()
        self.buf.append(var)
        if len(self.buf) > self.cap:
            self.buf.pop(0)
        hot = len(self.buf) == self.cap and sum(self.buf)/self.cap > self.thresh
        debug_print(f"EntropyWindow: var={var:.4f}, avg={sum(self.buf)/len(self.buf):.4f}, hot={hot}")
        return hot
...
class CVMPTransformer(nn.Module):
    def __init__(self, cfg: CVMPConfig):
        ...
        self.heal_budget = 0        # how many steps of reset left
        self.heal_cool = 0          # cooldown before next heal can arm
        self.bloom_catch = BloomCatch()
        self.tier_drift = TierDriftMonitor()  # New: Tier drift monitor
        self.ent = EntropyWindow()  # New: Entropy window
        
        debug_print(f"CVMPTransformer initialized with {cfg.n_layers} layers")

    def soft_reset(self):
        debug_print(f"Performing soft reset (heal_budget: {self.heal_budget})")
        
        # Adaptive gain with 90-100% retention
        gain = 0.90 + 0.10 * torch.rand(1).item()
        debug_print(f"Using gain factor: {gain:.4f}")
        ...
        self.heal_budget -= 1
        self.heal_cool = max(self.heal_cool, 6)  # at least 6 passes idle
        debug_print(f"Reset {param_count} parameters. Decreasing heal_budget from {self.heal_budget+1} to {self.heal_budget}, cool={self.heal_cool}")
    ...
    def forward(self, input_ids: torch.Tensor, mask: torch.Tensor = None, return_trace: bool = False):
        ...
        debug_print(f"Current heal_budget: {self.heal_budget}, heal_cool: {self.heal_cool}")
        ...
        # Check entropy window before triggering
        entropy_hot = self.ent.update(logits)
        if entropy_hot and self.heal_budget == 0:
            debug_print("[Entropy] window hot – arming heal")   
            self.heal_budget = round(min(5, 1 + 10*(sum(self.ent.buf)/len(self.ent.buf))))
        
        # Main healing logic
        if self.heal_budget > 0:
            self.soft_reset()
        elif self.heal_cool > 0:
            self.heal_cool -= 1
            debug_print(f"Cooling down: {self.heal_cool} steps remaining")
        else:
            # normal operation; heal budget can be re-armed
            if bloom_triggered or entropy_hot:
                self.heal_budget = min(5, 1 + int(self.cfg.dps * 2))   # scale to DPS
                debug_print(f"Arming heal budget: {self.heal_budget} (bloom={bloom_triggered}, entropy={entropy_hot})")
        
        # Update tier drift monitor
...
if __name__ == "__main__":
    ...
    # Test entropy window triggering
    print("\n" + "-"*50)
    print("Testing entropy window...")
    print("-"*50)
    
    # Generate a sequence that should trigger high entropy
    for i in range(10):
        print(f"\nEntropy test pass {i+1}...")
        test_seq = torch.randint(0, cfg.vocab_size, (1, 16))
